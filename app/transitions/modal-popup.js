...
function hideModal(oldView) {
  ...
  // ]);
	...
	return Promise.all([
		Velocity.animate(obscure, {opacity: [0, 0.5]}, {duration: 350}),
		Velocity.animate(box, {translateY: "-100%", opacity: [0, 1]}, {duration: 350})
	]);
}
  ...
  return insertNewView().then(function(newView){
    ...
    return Promise.all([
      ...
      // Velocity.animate(box, {scale: [1, 0]}, {duration: 250, display: ''})
  		Velocity.animate(obscure, {opacity: [0.5, 0]}, {duration: 250, display: ''}),
			// Slide Down
		//Velocity.animate(box, {translateY: ["0%", "-100%"], opacity: [1, 0]}, {duration: 250, display: ''})
			// Fade in 
		Velocity.animate(box, {translateY: "0%", opacity: [1, 0]}, {duration: 250, display: ''})
    ]);
  });
}
